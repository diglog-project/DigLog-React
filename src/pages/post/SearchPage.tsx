                ...
                <div className='w-full flex flex-col max-w-xl mx-auto'>
                    <div className='flex'>
                        <div className='w-full relative flex justify-between items-center'>
                            ...
                            <button
                                className={`${
                                    searchRequest.keyword === '' ? 'hidden' : ''
                                } absolute right-16 rounded-full hover:cursor-pointer hover:bg-gray-1000`}
...
function SearchTab({ selectedTab, setSelectedTab }: { selectedTab: string; setSelectedTab: (tab: string) => void }) {
    const tabs = [
        { key: '게시글', value: 'post' },
        { key: '블로그', value: 'blog' },
    ];
                        ...
                        <button
                            ...
                            className={`${
                                selectedTab === tab.value
                                    ? 'border-b-2 text-lime-400 border-lime-400'
                                    : 'hover:border-b-2 hover:text-lime-400 hover:border-lime-400'
                            } inline-block p-4 hover:cursor-pointer`}
                        >
                            {tab.key}
                        </button>
                    </div>
        ...
        return (
            <div className='my-2'>
                ...
                {postPageInfo.number + 1 < postPageInfo.totalPages && (
                    <LoadMoreButton onClick={handlePostPage} addStyle={'w-full'} />
                )}
            </div>
        );
        ...
        return (
            <div className='flex flex-col divide-y divide-gray-200 z-0'>
                ...
                {blogPageInfo.number + 1 < blogPageInfo.totalPages && (
                    <LoadMoreButton onClick={handleBlogPage} addStyle={'w-full'} />
                )}
            </div>
        );
    }
            ...
            <div className='flex justify-start items-center gap-x-4'>
                <div className='relative'>
                    ...
                    <div
                        className={`${
                            open ? '' : 'hidden'
                        } absolute top-10 left-0 flex flex-col z-10 bg-white divide-y divide-gray-100 rounded-lg shadow-sm w-44`}
                    >
...
function SortMenu({
    ...
    return (
        <button
            ...
            className={`${
                value === menu.value ? 'bg-gray-100' : ''
            } inline-flex w-full px-4 py-2 hover:bg-gray-200 hover:cursor-pointer`}
        >
            {menu.key}
        </button>
    );
