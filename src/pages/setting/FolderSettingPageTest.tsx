...
function FolderSettingPage() {
    ...
    const [folderEditType, setFolderEditType] = useState<string>(""); // create, delete
    ...
    return (
        <div>
            <p className="font-semibold text-xl my-4">폴더 관리 테스트</p>
            <FolderCardListTest
                folders={folders}
            ...
            <div className="flex justify-end gap-x-4 mt-4">
                {folderEditType !== "create" &&
                    <FillButton text={"생성/수정 관리"} onClick={() => handleFolderEditType("create")}
                                addStyle={"text-sm bg-lime-700"}/>}
                {folderEditType !== "delete" &&
                    <FillButton text={"삭제 관리"} onClick={() => handleFolderEditType("delete")}
                                addStyle={"text-sm bg-red-500"}/>}
                {folderEditType !== "" && <FillButton text={"변경사항 저장"} onClick={handleSubmit} addStyle={"text-sm"}/>}
            </div>
            {openMoveModal && (
