            ...
            <div className='flex flex-col gap-y-2 my-4'>
                <div className='flex justify-between items-center'>
                    <div className='flex justify-center items-center gap-x-4 text-sm'>
                        <Link to={`/blog/${post.username}`} className='font-semibold text-lime-700 hover:text-lime-400'>
                            {post.username}
                        </Link>
                        <div className='text-gray-500 font-normal'>{relativeDateToKorean(post.createdAt)}</div>
                    </div>
                ...
                <Link to={`/post/${post.id}`}>
                    <div className='line-clamp-2 hover:text-gray-600 font-medium'>{post.title}</div>
                </Link>
                <div className='line-clamp-3 text-gray-500 text-sm font-normal'>{removeHtmlTags(safeContent)}</div>
            </div>
        </div>
    );
}
